...
| `LLDAP_BACKUP_RETENTION_DAYS` | The amount of time in days since an LLDAP backup has been last modified until it is deleted. | `7` | `2` |
...
### Authelia
 ...
 - Backup of all data from the `authelia-data` volume within the container is scheduled for 01:30 by the default value of `AUTHELIA_BACKUP_SCHEDULE` and written to `/backups`
   - `/backups` must be backed up via volume mounted from `${VOLUME_DIR}/authelia-backups`

### LLDAP

 - Backup of all data from the `lldap-data` volume within the container is scheduled for 01:30 by the default value of `LLDAP_BACKUP_SCHEDULE` and written to `/backups`
   - `/backups` must be backed up via volume mounted from `${VOLUME_DIR}/lldap-backups`
